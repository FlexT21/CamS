...
Los dos proyectos incluyen configuración de Ruff. Para revisar el código:
...
Actualmente el repositorio no incluye una suite de pruebas automatizadas. Se recomienda probar como mínimo el healthcheck, la conexión WebSocket, un caso con rostro desconocido y un caso con un usuario registrado antes de desplegar cambios.

## Solución de problemas

### `SERVER_CORS_ORIGINS` no está configurada

Compose requiere esta variable. Verifica que exista `.env` en la raíz y que contenga, como mínimo:

```dotenv
SERVER_CORS_ORIGINS='["*"]'
```

### El cliente no conecta al WebSocket

Comprueba que el servidor esté escuchando en el puerto correcto y que la URL incluya el prefijo y la ruta completos:

```text
ws://localhost:8765/api/ws/
```

Si el servidor está dentro de Docker y el cliente fuera, usa `localhost`. El nombre `server` solo es resoluble desde otro contenedor de la misma red de Compose.

### El usuario aparece como `Unknown`

Comprueba que la carpeta del usuario exista, que las extensiones estén incluidas en `VALID_IMAGE_EXTENSIONS` y que las fotografías contengan un rostro claro. Ajusta `THRESHOLD_DISTANCE` con prudencia: aumentarlo facilita coincidencias, pero también puede aumentar falsos positivos.

### Docker falla durante la construcción del servidor

El `Dockerfile` actual usa una imagen base Python 3.10, mientras que ambos `pyproject.toml` declaran Python 3.12. Si el resolvedor de dependencias rechaza la versión de Python, alinea la imagen base y los metadatos del proyecto antes de volver a ejecutar:

```powershell
docker compose build --no-cache server
docker compose up server mosquitto
```

## Seguridad y privacidad

Las imágenes de usuarios son datos biométricos. No las subas al repositorio ni las expongas mediante un volumen compartido sin controles de acceso. Para un entorno real, configura autenticación, TLS para WebSocket y MQTT, una lista explícita de orígenes CORS y credenciales para el broker.
